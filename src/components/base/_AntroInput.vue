<template>
  <div class="antro-input" :class="[required ? 'required' : '']">
    <label v-if="label" class="antro-input__label" @click="setFocus">
      {{ label }}
      <span v-if="!isFocused">{{ prettyValue }}</span>
    </label>
    <input
...
export default {
  ...
  methods: {
    createLinePath() {
      const startingPoint = 8 // set to 8 since this is the end point
    ...
    focus(event) {
      const path = this.$el.querySelector('.antro-input__line path')
      const input = this.$el.querySelector('.antro-input__input')
      const vm = this

      const timeline = anime.timeline()

      if (this.label !== '') {
        const label = this.$el.querySelector('.antro-input__label')
